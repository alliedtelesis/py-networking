...
class ats_system(object):
    ...
    def get_startup_config(self):
        self._device.log_info('getting device configuration')
        config = ''
        for line in self._device.cmd('show startup-config').replace('\r', '').split('\n'):
            config += line + '\n'
        self._device.log_debug('got device configuration \n{0}'.format(config))
        return config
    ...
    def update_firmware(self, filename, protocol='http', server='', port=69, dontwait=True):
        ...
        self._device.file.create(name='image', protocol=protocol, filename=filename, server=server, port=port)
    ...
    def _update_bank_data(self):
        ...
        ifre = re.compile('(?P<unit>\d+)\s+'
                          ...
                          '(?P<time>[^\s]+)\s+'
                          '(?P<status>[^\s]+)\s+')
        for line in self._device.cmd("show bootvar").split('\n'):
            m = ifre.match(line)
            self._device.log_debug("read {0}".format(line))
            if m:
                if (m.group('status')[:6] == 'Active'):
                    self._device.log_debug("active bank is {0}".format(m.group('image')))
                else:
    ...
    def _get_stand_by_bank(self):
        self._device.log_debug("stand by bank is {0}".format(self._stand_by_bank))
        return self._stand_by_bank

    def _is_boot_bank_changed(self):
        is_changed = False
        self._update_bank_data()
        self._device.log_debug("old boot bank is {0}".format(self._old_boot_bank))
        self._device.log_debug("new boot bank is {0}".format(self._new_boot_bank))
