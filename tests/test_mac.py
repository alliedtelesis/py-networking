...
"""]
    ...
    dut.add_cmd({'cmd': delete_cmd_1            , 'state':7, 'action':'SET_STATE','args':[8]})
    dut.add_cmd({'cmd': delete_cmd_2            , 'state':8, 'action':'SET_STATE','args':[9]})
    dut.add_cmd({'cmd': delete_cmd_3            , 'state':9, 'action':'SET_STATE','args':[10]})
    dut.add_cmd({'cmd': 'show mac address-table', 'state':10,'action':'PRINT'    ,'args': output_0})
